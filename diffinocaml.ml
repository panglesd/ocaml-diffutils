type atom = string
type file = atom list
type numbered_file = (int * atom) list

open StdLabels

let rec end_string n l =
  if n <= 0 then l
  else
    match l with
    | [] -> failwith ("incorrect substring " ^ string_of_int n)
    | _ :: q -> end_string (n - 1) q

let size_common_prefix a b =
  let rec size_common_prefix n a b =
    match (a, b) with
    | (_, ta) :: qa, (_, tb) :: qb when String.equal ta tb ->
...
exception Found2 of int

let v_pp ppf v =
  Printf.fprintf ppf "%s"
    (v |> Array.to_list |> List.map ~f:string_of_int |> String.concat ~sep:" ")

(* let diff a b = *)
(*   let a = List.mapi ~f:(fun i x -> (i, x)) a *)
(*   and b = List.mapi ~f:(fun i x -> (i, x)) b in *)
(*   let n = List.length a and m = List.length b in *)
(*   let max = n + m in *)
(*   let v = Array.make ((2 * max) + 1) 0 in *)
(*   let ret = ref [] in *)
(*   for d = 0 to max do *)
(*     Printf.printf "Stage %d\n" d; *)
(*     let k = ref (-d) in *)
(*     while !k <= d do *)
(*       Printf.printf " k is %d\n" !k; *)
(*       let x = *)
(*         if !k = -d || (!k != d && v.(!k - 1 + max) < v.(!k + 1 + max)) then *)
(*           v.(!k + 1 + max) *)
(*         else v.(!k - 1 + max) + 1 *)
(*       in *)
(*       let y = x - !k in *)
(*       if x > n || y > m then () *)
(*       else ( *)
(*         Printf.printf "  x and y are %d %d\n" x y; *)
(*         let tempa = end_string x a and tempb = end_string y b in *)
(*         Printf.printf "  tempa and tempb are %s %s\n" *)
(*           (String.concat ~sep:"" (List.map ~f:snd tempa)) *)
(*           (String.concat ~sep:"" (List.map ~f:snd tempb)); *)
(*         Printf.printf "  SCP tempa tempb %d\n" (size_common_prefix tempa tempb); *)
(*         v.(!k + max) <- x + size_common_prefix tempa tempb; *)
(*         let new_x = x + size_common_prefix tempa tempb in *)
(*         let new_y = new_x - !k in *)
(*         if new_x >= n && new_y >= m then raise (Found (d, !ret)); *)
(*         ()); *)
(*       k := !k + 2 *)
(*     done; *)
(*     Printf.printf " v is %a\n" v_pp v; *)
(*     ret := Array.copy v :: !ret *)
(*   done; *)
(*   raise Not_found *)

type instruction = Diag of int | Down | Right

type point =
  (int * int) * ((int * string) list * (int * string) list) * instruction list

exception Found3 of point * int

let follow_diagonal ((x, y), (a, b), il) =
  let n, (a, b) = common_prefix a b in
  let il = if n = 0 then il else Diag n :: il in
  ((x + n, y + n), (a, b), il)

let is_a_win = function _, ([], []), _ -> true | _ -> false

let pp_state ppf state =
  let s =
    List.map state ~f:(fun ((x, _), _) -> string_of_int x)
    |> String.concat ~sep:" "
  in
  Printf.fprintf ppf "%s" s

let diff a b =
  let rec step d state =
    let rec next state =
      match state with
      | [] -> []
      | [ ((x, y), (_ :: a, b), il) ] ->
          let p = follow_diagonal ((x + 1, y), (a, b), Right :: il) in
          if is_a_win p then raise (Found3 (p, d));
          [ p ]
      | ((x1, y1), ([], b1), _) :: (((x2, y2), (a2, t2 :: b2), il2) :: _ as q)
        ->
          let p = follow_diagonal ((x2, y2 + 1), (a2, b2), Down :: il2) in
          if is_a_win p then raise (Found3 (p, d));
          p :: next q
      | ((x1, y1), (_ :: a1, b1), il1) :: ((_, (_, []), _) :: _ as q) ->
          let p = follow_diagonal ((x1 + 1, y1), (a1, b1), Right :: il1) in
          if is_a_win p then raise (Found3 (p, d));
          p :: next q
      | ((x1, y1), (t1 :: a1, b1), il1)
        :: (((x2, y2), (a2, t2 :: b2), il2) :: _ as q) ->
          let p =
            if x1 >= x2 then
              follow_diagonal ((x1 + 1, y1), (a1, b1), Right :: il1)
            else follow_diagonal ((x2, y2 + 1), (a2, b2), Down :: il2)
          in
          if is_a_win p then raise (Found3 (p, d));
          p :: next q
      | ((_, _), ([], _), _) :: (_, (_, []), _) :: _ -> failwith "impossible"
      | [ ((x, y), ([], b), _) ] -> []
    in
    (* Printf.printf "state is %a\n" pp_state state; *)
    let state =
      match state with
      | [] -> []
      | ((x1, y1), (_, []), _) :: _ -> next state
      | ((x1, y1), (a1, _ :: b1), il1) :: _ ->
          let p = ((x1, y1 + 1), (a1, b1), Down :: il1) in
          let p = follow_diagonal p in
          if is_a_win p then raise (Found3 (p, d));
          p :: next state
    in
    step (d + 1) state
  in
  let p = follow_diagonal ((0, 0), (a, b), []) in
  if is_a_win p then raise (Found3 (p, 0));
  step 1 [ p ]

let diff a b =
  let a = List.mapi ~f:(fun i x -> (i, x)) a in
  let b = List.mapi ~f:(fun i x -> (i, x)) b in
  try diff a b with Found3 (p, d) -> (p, d)
...
module V2 = struct
  ...
  let register_transformation_using_ocaml_current_ast ?impl ?intf ?aliases name
      =
    let impl =
      Option.map impl ~f:(fun f ctx ast ->
          ast
    ...
    in
    let intf =
      Option.map intf ~f:(fun f ctx ast ->
          ast
...
let a = String.split_on_char ~sep:'\n' a
let b = String.split_on_char ~sep:'\n' b
...
(* let backtrack a b ret = *)
(*   let x = List.length a and y = List.length b in *)
(*   let ret = *)
(*     ret *)
...
(*            let k = x - y in *)
(*            let prev_k = *)
(*              if k = -d || (k != d && v.(k - 1) < v.(k + 1)) then k + 1 *)
(*              else k - 1 *)
(*            in *)
(*            (v.(prev_k), v.(prev_k) - prev_k) *)
(*            (\* (d, v, "aaaaaaaaaaaaaaaaaaaaaaaaaaaa") *\)) *)
(*          ~init:(x, y) *)
(*   in *)
(*   () *)

(* let c = [ "C"; "B"; "A"; "B"; "A"; "A" ] *)
(* let () = Printf.printf "SCP(a,b) = %d\n" (size_common_prefix a b) *)
(* let () = Printf.printf "SCP(b,c) = %d\n" (size_common_prefix b c) *)
let (_, _, l), d = diff a b
let () = Printf.printf "The result is %d\n" d

let () =
  print_string "Original_line: ";
  List.iter ~f:print_string a;
  print_string "\nDestination_line: ";
  List.iter ~f:print_string b;
  print_newline ();
  l |> List.rev
  |> List.map ~f:(function
       | Right -> "Remove a line"
       | Down -> "Add a line"
       | Diag n -> Printf.sprintf "Keep %d line(s)" n)
  |> List.iter ~f:print_endline
(* let diff3 diff base s1 s2 = diff + base + s1 + s2 *)
...
let rec print a b l =
  ...
  | e :: a, b, Right :: l ->
      ...
      print a b l
  | e :: a, _ :: b, Diag n :: l ->
      let l = if n = 1 then l else Diag (n - 1) :: l in
      Printf.printf "%s\n" e;
      print a b l
  | _ -> failwith "a b and l do not match"

let () = print a b (List.rev l)
(*   print_string "Original_line: "; *)
(*   List.iter ~f:print_string a; *)
(*   print_string "\nDestination_line: "; *)
(*   List.iter ~f:print_string b; *)
...
(* type merge_instruction = *)
(*   | Common of string list *)
(*   | Merged of string list *)
(*   | Conflict of string list * string list * string list *)

(* type merge_instructions = Keep of int *)

(* let diff3 diff mine original yours = *)
(*   let _, il_mine = diff original mine and _, il_yours = diff original yours in *)
(*   let rec eat_conflict acc_mine acc_yours il_mine o_mine il_yours o_yours = *)
(*     (\* if o_mine < o_yours then *\) *)
(*     (\*   match il_mine with Diag n :: il_mine -> _ | _ -> _ *\) *)
(*     (\* else _ *\) *)
(*     match (il_mine, il_yours) with *)
(*     | Diag 0 :: il_mine, _ -> *)
(*         eat_conflict acc_mine il_mine o_mine il_yours o_yours *)
(*     | _, Diag 0 :: il_yours -> *)
(*         eat_conflict acc_yours il_mine o_mine il_yours o_yours *)
(*     | Diag n :: il_mine, _ when o_mine < o_yours -> *)
(*         let acc = Keep n :: acc_mine in *)
(*         eat_conflict acc_mine acc_yours il_mine o_mine il_yours o_yours *)
(*     | _ -> _ *)
(*   in *)
(*   let rec eat_match acc il_mine o_mine il_yours o_yours = *)
(*     match (il_mine, il_yours) with *)
(*     | Diag 0 :: il_mine, _ -> eat_match acc il_mine o_mine il_yours o_yours *)
(*     | _, Diag 0 :: il_yours -> eat_match acc il_mine o_mine il_yours o_yours *)
(*     | Diag n :: il_mine, Diag m :: il_yours when o_mine = o_yours -> *)
(*         let min = if n < m then n else m in *)
(*         let acc = Keep min in *)
(*         let il_mine = *)
(*           if n - min = 0 then il_mine else Diag (n - min) :: il_mine *)
(*         in *)
(*         let il_yours = *)
(*           if n - min = 0 then il_yours else Diag (n - min) :: il_yours *)
(*         in *)
(*         eat_match acc il_mine (o_mine + min) il_yours (o_yours + min) *)
(*     | _ -> _ *)
(*   in *)
(*   _ *)
