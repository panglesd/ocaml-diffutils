...
module Patch = struct
  ...
  let common_prefix a b =
    let rec size_common_prefix n a b =
      match (a, b) with
      | ta :: qa, tb :: qb when String.equal ta tb ->
  ...
  let git_printer =
    {
      keep = (fun ppf -> Fmt.pf ppf " %s\n");
      add = (fun ppf -> Fmt.pf ppf "+%s\n");
      remove = (fun ppf -> Fmt.pf ppf "-%s\n");
      sep = Fmt.nop;
      context = 3;
    }
...
module Diff = struct
  ...
  let html_printer =
    ...
    in
    {
      same =
        (fun ppf a ->
          let pp_common = elem_class "common-line" Fmt.string in
          elem_class "common"
      ...
      diff =
        (let pp_removed =
           elem_class "removed "
             (Fmt.list ~sep:Fmt.cut (elem_class "removed-line" Fmt.string))
         and pp_added =
           elem_class "added"
             (Fmt.list ~sep:Fmt.cut (elem_class "added-line" Fmt.string))
         in
...
module Diff3 = struct
  ...
  let to_inputs (diff3 : t) =
    ...
    |> List.map ~f:(function
         ...
         | Diff { base; you; me } ->
             ...
             (base, you, me))
    |> split3 [] [] []
    |> fun (base, you, me) -> (List.concat base, List.concat you, List.concat me)
  ...
  let git_printer =
    let pp fmt = Fmt.pf fmt "%a\n" Fmt.string in
    {
      ...
      diff =
        (fun ppf { me; base; you } ->
          let pp_l = Fmt.(list (Fmt.string ++ const Fmt.char '\n')) in
          let you = Patch.apply base you and me = Patch.apply base me in
          Fmt.pf ppf {|>>>
%a|||
%a===
...
module Merge = struct
  ...
  let total_merge merge resolve =
    merge
    |> List.map ~f:(function Resolved s -> [ s ] | Unresolved u -> resolve u)
    |> List.concat
