(** A word on terminology.
    ...
    - the Lehvenstein distance, where deletion, addition and substitution are
      allowed (See the
      {{:https://v2.ocaml.org/api/compilerlibref/Diffing.html} OCaml module
        Diffing})
...
module Patch : sig
  ...
  (** In LCS (Longest Common Subsequence), the only operations allowed are
      ...
      we omit information already present in the original sequence, so {!Remove}
      takes an integer. *)
  type hunk = Keep of int | Remove of int | Add of string
...
module Reversible_patch : sig
  (** A patch is a minimal information to get from a sequence of values of type
      {!string} to another, and vice versa. *)

  (** In LCS (Longest Common Subsequence), the only operations allowed are
      {!Keep} {!Remove} and {!Add}. Contrary to the usual [diff/patch] format,
      we omit information already present in the original sequence, so {!Remove}
      takes an integer. *)
  type hunk = Keep of int | Remove of string | Add of string
...
module Conflict : sig
  ...
  (** The reason [you] and [me] are [patch] and not [input] is to be able to
      quickly check if one is equal to [base]. [you] and [me] as {!input} can be
      recovered using {!Patch.apply} with [base]. *)
end

(** A diff3 defines 3 lists and how they relate to each other. *)
module Diff3 : sig
  type hunk = Same of string | Diff of Conflict.t
  ...
  type printer = { same : string Fmt.t; diff : Conflict.t Fmt.t }
...
module Patch3 : sig
  ...
  (** Applying a [patch3] will give three files, which will be given as a value
      of type {!diff3}. *)
end

(** Merges of three lists *)
module Merge : sig
  type hunk = Resolved of string | Unresolved of Conflict.t
  ...
  val merge :
    ...
    unit ->
    t
  (** Tries to merge two files given a base. The optional [resolve] argument can
      resolve some parts and let others unresolved. By default, it resolves the
      case where either {!diff3_diff.you} or {!diff3_diff.me} is empty (as git
      does). *)

  val total_merge : t -> total_resolver -> input
  (** [total_merge u f] calls [f] on each [`Conflict] to resolve them *)

  type printer = Diff3.printer = {
    same : string Fmt.t;
    diff : Conflict.t Fmt.t;
  }
